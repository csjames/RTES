...
public class SupportFragmentTabListener<T extends Fragment> implements ActionBar.TabListener {
    ...
    public SupportFragmentTabListener(int fragmentContainerId, MainActivity activity, String tag, Class<T> clz) {
        mActivity = activity;
        mTag = tag;
        mClass = clz;
    ...
    public void onTabSelected(Tab tab, FragmentTransaction sft) {
        ...
        if (mFragment == null) {
            ...
            if (mFragment instanceof ConnectionInfo){
                mActivity.register((ConnectionInfo) mFragment);
            }
        } else {
